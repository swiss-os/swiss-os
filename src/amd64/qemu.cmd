...
qemu-system-x86_64 -D qemu.log -d int,cpu_reset -no-reboot -drive if=pflash,format=raw,unit=0,readonly=on,file="/usr/share/qemu/OVMF.fd" -drive if=none,id=stick,format=vpc,file=32gears.vhd -device nec-usb-xhci,id=xhci,p2=4,p3=1 -device usb-storage,drive=stick,bus=xhci.0

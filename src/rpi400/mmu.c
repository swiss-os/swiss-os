...
static os_uint64 *create_l2(os_uint32 n, os_uint64 a)
{
    ...
    k__printf("L2 base: %x:%x\n", (os_uint32)(a >> 32), (os_uint32)a);
    ...
    for (i = 0; i < 512; i++)
    {
        ...
        set_l2_AttrIndx(l2 + i, ATTRINDX_NORMAL);
        /*set_l2_AP(l2 + i, ATTRIB_AP_RW_ALL)*/
        set_l2_AP(l2 + i, ATTRIB_AP_RW_PL1);
        set_l2_SH(l2 + i, ATTRIB_SH_INNER_SHAREABLE);
        set_l2_AF(l2 + i, 1);
     
        if ((a >= MEM_LOWMEM_END && a < MEM_HIGHMEM_START) ||
            a > MEM_HIGHMEM_END) 
        {
            set_l2_PXN(l2 + i, 1);
            set_l2_AttrIndx(l2 + i, ATTRINDX_DEVICE);
            set_l2_SH(l2 + i, ATTRIB_SH_OUTER_SHAREABLE);
        } else if (a >= MEM_COHERENT_REGION && a < MEM_HEAP_START) {
            set_l2_PXN(l2 + i, 1);
            set_l2_AttrIndx(l2 + i, ATTRINDX_COHERENT);
            set_l2_SH(l2 + i, ATTRIB_SH_OUTER_SHAREABLE);
        }

...
int parser__is_typedef_name(struct parser *self, struct token *tk)
{
	// FIXME 
	if (0) {
		tk->type = token__TYPEDEF_NAME;
	}
	return 0;
}

int parser__is_enumeration_constant(struct parser *self, struct token *tk)
{
	// FIXME 
	if (0) {
		tk->type = token__ENUMERATION_CONSTANT;
	}
	return 0;
}
...
int parser__is_end_of_many(struct parser *self)
{
	struct prediction *p;
